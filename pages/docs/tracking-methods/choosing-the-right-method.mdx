...
- Offline data is stored within cache or local storage, it will be sent to the ingestion server upon reconnection to the internet, and the ingestion server will accept offline historical data with a timestamp that is within the last 5 days.
